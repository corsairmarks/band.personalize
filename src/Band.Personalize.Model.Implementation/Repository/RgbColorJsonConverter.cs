...
namespace Band.Personalize.Model.Implementation.Repository
{
    ...
    /// <summary>
    /// Converts an <see cref="RgbColor"/> to and from JSON.
    /// </summary>
    public class RgbColorJsonConverter : JsonConverter
    {
        /// <summary>
        ...
        {
            RgbColor color;
            return RgbColor.TryFromRgbString(serializer.Deserialize<string>(reader), out color)
                ? color
                : null;
        }
        ...
        {
            writer.WriteValue(value != null ? ((RgbColor)value).ToString() : string.Empty);
        }
    }
}